##Old Market Design - Stage 1 - Operational Case
import numpy as np
import pandas as pd
...
def calculate_S1 (optimal_quantiles,p_obs,dam_price, ida_price,exc_price,def_price, afrr_price, afrr_nup, afrr_ndown,afrr_upused, afrr_downused, trup_price, trdown_price,excess,deficit,\
    ...
    if dev < 0: ## Negative deviation in IDA
        ...
        if imbalance > 0:
            ...
            total_rev = dam_rem + ida_rem + dev_rem
        else:
            def_b = abs(imbalance)
        ...
        if afrr_nup > 0 and optimal_quantiles > min_bid and dev > min_bid:
            ...
            down_cap = min(dev/2, afrr_ndown, optimal_quantiles, afrr_nup/2)
            ...
            if avcap >= up_req: ## Available capacity is higher than requested for UP activation
                ...
                if up_req > (down_req*2) or down_req == 0: ## Activation only in UP direction
                    ...
                    if imbalance > 0:
                        if excess > 0:
                            ...
                            total_rev = dam_rem + cap_rem + en_rem + dev_rem+ ida_rem
                        else:
                            curt = imbalance
                            total_rev = dam_rem + cap_rem  + en_rem+ ida_rem
                    else:
                        def_b1 = abs(imbalance)
                        def_rem = - def_b1 * deficit
                        total_rev = dam_rem + cap_rem + en_rem + def_rem + ida_rem
                ...
                if avcap > 0:
                    ...
                    def_rem = - def_b1 * deficit
                    total_rev = dam_rem + ida_rem +  cap_rem + en_rem + def_rem
         
                else:
                    def_b1 = abs(p_obs - prog - up_req)
                    def_rem = - def_b1 * deficit
                    total_rev = dam_rem + ida_rem + cap_rem + def_rem
